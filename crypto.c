...
#ifdef WITH_FREERTOS
static SemaphoreHandle_t cipher_context_mutex = 0;
#elif defined(WITH_CONTIKI)
...
{
#ifdef WITH_FREERTOS
	xSemaphoreTake(cipher_context_mutex, ( TickType_t ) 100);
#elif defined(WITH_CONTIKI)
...
static dtls_handshake_parameters_t *dtls_handshake_malloc(void) {
  return osMalloc(sizeof(dtls_handshake_parameters_t));
}
...
static dtls_security_parameters_t *dtls_security_malloc(void) {
  return osMalloc(sizeof(dtls_security_parameters_t));
}
...
dtls_ecdsa_create_sig_hash(const unsigned char *priv_key, size_t key_size,
  ...
  do {
    if(dtls_prng((unsigned char *)rand, key_size) == -1){
    	dtls_prng_init();
    	continue;
    }
