...
static QueueHandle_t process_event_timer = 0;
static TickType_t retransmit_timer = portMAX_DELAY;
static TaskHandle_t dtls_retransmit_task = 0;

static void DEL_PEER(dtls_peer_t *head, dtls_peer_t *delptr);

static void dtls_retransmit_process(void *pvParameters);

static inline dtls_context_t* malloc_context(void) {
	return (dtls_context_t*) osMalloc(sizeof(dtls_context_t));
}
 ...
 * client
 */
static int hs_attempt_with_existing_peer(uint8_t *msg, size_t msglen,
		dtls_peer_t *peer) {
	(void) msglen;
	if ((peer) && (peer->state == DTLS_STATE_CONNECTED)) {
		if (msg[0] == DTLS_CT_HANDSHAKE) {
			...
			if (msg_epoch == 0) {
				dtls_handshake_header_t *hs_header = DTLS_HANDSHAKE_HEADER(
						msg + DTLS_RH_LENGTH);
...
static int dtls_check_tls_extension(dtls_peer_t *peer, uint8 *data,
	...
	while (data_length) {
		...
		case TLS_EXT_ENCRYPT_THEN_MAC:
			...
			break;
		default:
			dtls_warn("unsupported tls extension: %d\r\n", i);
			break;
		}
		data += j;
		data_length -= j;
...
static int dtls_send_handshake_msg_hash(dtls_context_t *ctx, dtls_peer_t *peer,
	...
	dtls_debug("send handshake packet of type: %s (%d)\r\n",
...
static int dtls_send_multi(dtls_context_t *ctx, dtls_peer_t *peer,
	...
	if ((type == DTLS_CT_HANDSHAKE
		...
		if (n) {
			dtls_tick_t now;
			dtls_ticks(&now);
			n->t = now + IOKEY_REPEAT_TIMEOUT * CLOCK_SECOND;
			n->retransmit_cnt = 0;
			n->timeout = IOKEY_REPEAT_TIMEOUT * CLOCK_SECOND;
			n->peer = peer;
...
#elif defined(WITH_FREERTOS) /* WITH_CONTIKI */
			} else {
				dtls_debug("copied to sendqueue\r\n");
				if (process_event_timer != 0)
					xQueueSend(process_event_timer, (void* )&ctx, 0);
#endif /* WITH_CONTIKI */
			}
		} else
...
static void dtls_destroy_peer(dtls_context_t *ctx, dtls_peer_t *peer,
	...
	if (unlink) {
		...
		struct UT_hash_handle *_hd_hh_del = (&(peer)->hh);
...
static int dtls_verify_peer(dtls_context_t *ctx, dtls_peer_t *peer,
		session_t *session, const dtls_state_t state, uint8 *data,
		size_t data_length) {
...
#define mycookie (buf + DTLS_HV_LENGTH)
	...
	if (len < 0) {
		dtls_warn("error while fetching the cookie, err: %d\r\n", err);
		return err;
	}
	...
	err = dtls_send_handshake_msg_hash(ctx,
			state == DTLS_STATE_CONNECTED ? peer : NULL, session,
...
static int check_client_certificate_verify(dtls_context_t *ctx,
	...
	if (ret < 0) {
		dtls_alert("wrong signature err: %d\r\n", ret);
		return dtls_alert_fatal_create(DTLS_ALERT_HANDSHAKE_FAILURE);
	}
	return 0;
}
...
static int dtls_send_server_hello_msgs(dtls_context_t *ctx, dtls_peer_t *peer) {
	...
	if (is_tls_ecdhe_ecdsa_with_aes_128_ccm_8(peer->handshake_params->cipher)) {
		...
		if (res < 0) {
			dtls_debug(
					"dtls_send_server_hello_msgs: cannot prepare Certificate record\r\n");
			return res;
		}
...
static int check_server_hello_verify_request(dtls_context_t *ctx,
	...
	//HAL_GPIO_TogglePin(DBG_GPIO_Port, DBG_Pin2);
	return res;
}
...
static int check_server_hellodone(dtls_context_t *ctx, dtls_peer_t *peer,
	...
	if (handshake->do_client_auth) {
		...
		if (res < 0) {
			dtls_debug(
					"check_server_hellodone: cannot prepare Certificate record\r\n");
			return res;
		}
	}
#endif /* DTLS_ECC */
	...
	if (handshake->do_client_auth) {
		...
		if (res < 0) {
			dtls_debug(
					"dtls_server_hello: verify_ecdh: cannot prepare Certificate record\r\n");
			return res;
		}
	}
#endif /* DTLS_ECC */
...
static int decrypt_verify(dtls_peer_t *peer, uint8 *packet, size_t length,
	...
	*cleartext = (uint8*) packet + sizeof(dtls_record_header_t);
	...
	if (!security) {
		dtls_alert("No security context for epoch: %d\r\n",
				dtls_get_epoch(header));
		return -1;
	}
...
static int handle_handshake_msg(dtls_context_t *ctx, dtls_peer_t *peer,
	...
	dtls_debug("handle handshake packet of type: %s (%d)\r\n",
	...
	case DTLS_HT_HELLO_VERIFY_REQUEST:
		...
		if (err < 0) {
			dtls_warn("error in check_server_hello_verify_request err: %d\r\n",
					err);
			return err;
		}
	...
	case DTLS_HT_SERVER_HELLO:
		...
		if (err < 0) {
			dtls_warn("error in check_server_hello err: %d\r\n", err);
			return err;
		}
		if (is_tls_ecdhe_ecdsa_with_aes_128_ccm_8(
				peer->handshake_params->cipher))
	...
	case DTLS_HT_CERTIFICATE:
		...
		if (err < 0) {
			dtls_warn("error in check_server_certificate err: %d\r\n", err);
			return err;
		}
		if (role == DTLS_CLIENT) {
	...
	case DTLS_HT_SERVER_KEY_EXCHANGE:
		...
		if (err < 0) {
			dtls_warn("error in check_server_key_exchange err: %d\r\n", err);
			return err;
		}
		peer->state = DTLS_STATE_WAIT_SERVERHELLODONE;
		/* update_hs_hash(peer, data, data_length); */
	...
	case DTLS_HT_SERVER_HELLO_DONE:
		...
		if (err < 0) {
			dtls_warn("error in check_server_hellodone err: %d\r\n", err);
			return err;
		}
	...
	case DTLS_HT_CERTIFICATE_REQUEST:
		...
		if (err < 0) {
			dtls_warn("error in check_certificate_request err: %d\r\n", err);
			return err;
		}
	...
	case DTLS_HT_FINISHED:
		...
		if (err < 0) {
			dtls_warn("error in check_finished err: %d\r\n", err);
			return err;
		}
		if (role == DTLS_SERVER) {
			/* send ServerFinished */
	...
	case DTLS_HT_CLIENT_KEY_EXCHANGE:
		...
		err = check_client_keyexchange(ctx, peer->handshake_params, data,
				data_length);
		if (err < 0) {
			dtls_warn("error in check_client_keyexchange err: %d\r\n", err);
			return err;
		}
		update_hs_hash(peer, data, data_length);
	...
	case DTLS_HT_CERTIFICATE_VERIFY:
		...
		if (err < 0) {
			dtls_warn("error in check_client_certificate_verify err: %d\r\n",
					err);
			return err;
		}
	...
	case DTLS_HT_CLIENT_HELLO:
		...
		if (err < 0) {
			dtls_warn("error in dtls_verify_peer err: %d\r\n", err);
			return err;
		}
...
static int handle_handshake(dtls_context_t *ctx, dtls_peer_t *peer,
	...
	dtls_debug("received handshake packet of type: %s (%d)\r\n",
	...
	if (dtls_uint16_to_int(hs_header->message_seq)
			< peer->handshake_params->hs_state.mseq_r) {
		dtls_warn(
				"The message sequence number is too small, expected %d, got: %d\r\n",
				peer->handshake_params->hs_state.mseq_r,
...
int dtls_handle_message(dtls_context_t *ctx, session_t *session, uint8 *msg,
	...
	int err;
	unsigned char multiple_packages = 0;
	...
	while ((rlen = is_record(msg, msglen))) {
		...
		//Give some delay in package handling for WDT
		if(multiple_packages > 0){
		...
		if (peer) {
			...
			dtls_security_parameters_t *security = dtls_security_params_epoch(
					peer, dtls_get_epoch(header));
			if (!security) {
				dtls_alert("No security context for epoch: %d\r\n",
						dtls_get_epoch(header));
				data_length = -1;
			} else {
				uint64_t pkt_seq_nr = dtls_uint48_to_int(
				...
				if (pkt_seq_nr == 0 && security->cseq.cseq == 0) {
					...
					if (((security->cseq.cseq - 1) - pkt_seq_nr) < 64) {
						if (security->cseq.bitfield
								& (1 << ((security->cseq.cseq - 1) - pkt_seq_nr))) {
							dtls_info(
							...
							data_length = decrypt_verify(peer, msg, rlen,
									&data);
							if (data_length > 0) {
								security->cseq.bitfield |= (1
										<< ((security->cseq.cseq - 1)
												- pkt_seq_nr));
							}
						}
					} else {
					...
					data_length = decrypt_verify(peer, msg, rlen, &data);
					if (data_length > 0) {
						security->cseq.bitfield <<= (pkt_seq_nr
								- security->cseq.cseq);
						security->cseq.bitfield |= 1
								<< ((pkt_seq_nr - security->cseq.cseq) - 1);
						security->cseq.cseq = pkt_seq_nr;
						dtls_debug(
								"new packet arrived with seq_nr: %d\r\n",
								(pkt_seq_nr));
						dtls_debug(
								"new bitfield is               : %d\r\n",
			...
			if (peer) {
				...
				if (expected_epoch != msg_epoch) {
					if (hs_attempt_with_existing_peer(msg, rlen, peer)) {
						...
						dtls_warn("Wrong epoch, expected %d, got: %d\r\n",
		...
		case DTLS_CT_APPLICATION_DATA:
			...
			dtls_stop_retransmission(ctx, peer);
			if (CALL(ctx, read, &peer->session, data, data_length) == -1) {
				return -1;
			}
			break;
		default:
...
#elif defined(WITH_FREERTOS)
	if (dtls_retransmit_task == 0) {
		xTaskCreate(dtls_retransmit_process, "dtls", TASK_DTLS_STACK, 0,
				TASK_DTLS_PRIORITY, &dtls_retransmit_task);
	}
	if (process_event_timer == 0) {
		process_event_timer = xQueueCreate(10, sizeof(dtls_context_t*));
		vQueueAddToRegistry( process_event_timer, "c8y" );
	}
	retransmit_timer = portMAX_DELAY;
#endif /* WITH_CONTIKI */
...
void dtls_free_context(dtls_context_t *ctx) {
    ...
    /*if (dtls_retransmit_task != 0) {
		vTaskDelete(dtls_retransmit_task);
		dtls_retransmit_task = 0;
	}
	if (process_event_timer != 0) {
		vQueueDelete(process_event_timer);
...
static void dtls_retransmit(dtls_context_t *context, netq_t *node) {
	...
	if (node->retransmit_cnt < DTLS_DEFAULT_MAX_RETRANSMIT) {
		...
		if (node->type == DTLS_CT_HANDSHAKE) {
			...
			dtls_debug("** retransmit handshake packet of type: %s (%d)\r\n",
					dtls_handshake_type_to_name(hs_header->msg_type),
					hs_header->msg_type);
		} else {
		...
		if (err < 0) {
			dtls_warn("can not retransmit packet, err: %d\r\n", err);
			return;
		}
...
static void dtls_retransmit_process(void *pvParameters) {
	...
	while (1) {
		if(xQueueReceive( process_event_timer, &ctx, portMAX_DELAY) == pdPASS )
		{
			if(ctx != 0 && ctx->sendqueue != 0){
				node = netq_head(&ctx->sendqueue);
				dtls_info("dtls_retransmit_process(%x,%x)\r\n",
												ctx, node);
				netq_pop_first(&ctx->sendqueue);
				netq_node_free(node);
				/*now = xTaskGetTickCount();


				if (node){
#if (DTLS_DEFAULT_MAX_RETRANSMIT != 1)
					if(node->t <= now) {
						dtls_info("netq_pop_first(%d)\r\n", node->retransmit_cnt);
						netq_pop_first(&ctx->sendqueue);
						dtls_retransmit(ctx, node);
						//netq_node_free(node);	//re-try only once
						//node = netq_head(&ctx->sendqueue);
					} else {
						xLastWakeTime = xTaskGetTickCount();
						vTaskDelayUntil(&xLastWakeTime, (node->t - now)/portTICK_RATE_MS);
						xQueueSend(process_event_timer, &ctx, 100);
					}
#else
					dtls_info("netq_pop_first(%d)\r\n", node->retransmit_cnt);
					netq_pop_first(&ctx->sendqueue);
...
int intepret_ecdsa_key(
    const unsigned char *key_string,
    size_t key_size,
		dtls_handshake_parameters_ecdsa_t *keyx
		) {
      int ret;
      ret = memcmp(key_string, cert_asn1_header, sizeof(cert_asn1_header));
  if (ret != 0) {
    dtls_alert("intepret_ecdsa_key error:%d, size: %d\n", ret, sizeof(cert_asn1_header));
    dtls_debug_dump("data\n", key_string, key_size);
    dtls_debug_dump("header\n", cert_asn1_header, sizeof(cert_asn1_header));
    return -1;
  }
  key_string += sizeof(cert_asn1_header);

  memcpy(keyx->other_pub_x, key_string,
	 sizeof(keyx->other_pub_x));
  key_string += sizeof(keyx->other_pub_x);

  memcpy(keyx->other_pub_y, key_string,
	 sizeof(keyx->other_pub_y));
  key_string += sizeof(keyx->other_pub_y);
...
static void DEL_PEER(dtls_peer_t *head, dtls_peer_t *delptr) {
	if ((head) != NULL && (delptr) != NULL) {
		...
		void *delptrhh = &(delptr)->hh;
		struct UT_hash_handle *_hd_hh_del = (delptrhh);
		if ((_hd_hh_del->prev == NULL) && (_hd_hh_del->next == NULL)) {
			HASH_BLOOM_FREE((head)->hh.tbl);
			uthash_free((head)->hh.tbl->buckets,
					(head)->hh.tbl->num_buckets * sizeof(struct UT_hash_bucket));
			(head)->hh.tbl->buckets = 0;
			...
			if (_hd_hh_del == (head)->hh.tbl->tail) {
				(head)->hh.tbl->tail = HH_FROM_ELMT((head)->hh.tbl,
						_hd_hh_del->prev);
			}
			...
			HASH_TO_BKT(_hd_hh_del->hashv, (head)->hh.tbl->num_buckets,
					_hd_bkt);
			if((head)->hh.tbl->buckets != 0){
				dbg_intf_printf(&dbg_intf_uart, 0, "DEL_PEER(%x,%x,%x,%x,%x)\r\n",
						head, (head)->hh, (head)->hh.tbl, _hd_bkt, _hd_hh_del);
